...
- `auto` and `minimum-savings` compare final rendered byte/token/line costs,
  ...
  explicitly `empty-success` and raw-safe; failed empty commands report
  `empty-command-failure`; nonempty content sanitized entirely away is lossy
  and omission-marked rather than empty-success.
- `full-if-bytes` and all presentation budgets are validated before spawn;
  ...
  unbounded provisional retention. Exact redactions are bounded by value
  count, aggregate bytes, per-value size, and conservative overlap-expansion
  budget. Tiny budgets that cannot represent a loss marker are rejected before
  spawn.
- A post-capture presentation/open failure is distinct from capture
  finalization failure: it reports `OUTCTL_POSTSPAWN_PRESENTATION_FAILED` in
  the post-spawn phase while retaining `capture_status=COMPLETE` and the
