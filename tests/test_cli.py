...
def test_cli_exposes_rollback_and_enablement_checks(
    ...
    evidence = Path(__file__).parents[1] / "config/enablement-evidence.example.json"
    ...
    assert main(["contract-validate", "enablement-evidence", str(evidence)]) == 0
    assert _payload(capsys) == {"schema": "enablement-evidence", "status": "VALID"}
